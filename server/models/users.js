...
export default class User {
  ...
  */
  ...
  create(values) {
    ...
    const sql = 'INSERT INTO users (branch_name, email, telephone, password, image_url, user_status) VAlUES( ${branch_name}, ${email}, ${telephone}, ${password}, ${image}, ${userStatus}) RETURNING id, branch_name, email, telephone, image_url, user_status';
...
*/
  ...
  findByStatus(status) {
    const sql = 'SELECT id, branch_name, email, telephone, image_url, created_at FROM users WHERE user_status = $1';
  ...
  */
  ...
  modify(values, id) {
    values.id = id;
    const sql = 'UPDATE users SET  branch_name=${branch_name}, email=${email}, telephone=${telephone} WHERE id=${id} RETURNING *';
    return this.db.one(sql, values);
  }
  /**
